...
import { BLOCK_GAS_LIMIT, TENDERLY_ACCESS_TOKEN, TENDERLY_BASE_URL, TENDERLY_ENCODE_URL, TENDERLY_SIM_URL } from '../constants'
import {
  generateProposalId,
  getGovernor,
  getProposal,
...
async function simulateProposed(config: SimulationConfigProposed): Promise<SimulationResult> {
  ...
  // block timestamp (represented by the `simTimestamp` variable below)
  const simTimestamp =
    governorType === 'bravo' ? BigNumber.from(latestBlock.timestamp).add(simBlock.sub(proposal.endBlock!).mul(12)) : proposal.endTime!.add(1)
  const eta = simTimestamp // set proposal eta to be equal to the timestamp we simulate at
