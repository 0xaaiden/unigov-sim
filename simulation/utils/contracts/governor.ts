...
export async function getProposal(governorType: GovernorType, address: string, proposalId: BigNumberish): Promise<ProposalStruct> {
...
export async function getProposalIds(governorType: GovernorType, address: string, latestBlockNum: number): Promise<BigNumber[]> {
  if (governorType === 'bravo') {
...
export async function generateProposalId(
  ...
  return BigNumber.from(
    keccak256(
      defaultAbiCoder.encode(['address[]', 'uint256[]', 'bytes[]', 'bytes32'], [targets, values, calldatas, keccak256(toUtf8Bytes(description))])
    )
  )
}
...
export function hashOperationOz(target: string, value: BigNumberish, calldata: string, predecessor: string, salt: string): BigNumber {
  return BigNumber.from(
    keccak256(defaultAbiCoder.encode(['address', 'uint256', 'bytes', 'bytes32', 'bytes32'], [target, value, calldata, predecessor, salt]))
  )
}
...
// For OZ governors, predecessor is often zero and salt is often description hash.
// This is only intended to be used with OZ governors.
export function hashOperationBatchOz(targets: string[], values: BigNumberish[], calldatas: string[], predecessor: string, salt: string): BigNumber {
  return BigNumber.from(
    keccak256(defaultAbiCoder.encode(['address[]', 'uint256[]', 'bytes[]', 'bytes32', 'bytes32'], [targets, values, calldatas, predecessor, salt]))
  )
}
